...
subtitle: |
  | ViDGER - An R package for the visualization of 
  | differential gene expression results of RNA-seq data
author: |
...
\footnotesize
\centering
\caption{An overview of the toy data sets included in this package. In this table, each data set is summarized in terms of what analytical software was used, organism ID, experimental layout (replicates and treatments), number of transcripts, and size of the data object in terms of megabytes (MB). Regarding the \texttt{DESeq2} data, two treatment conditions were considered in the data set, which reflects two separate sets of respective replications.}
\label{table1}
\begin{tabular}{@{}lllllll@{}}
\toprule
Data & Analytical Tool & Organism & Replicates & Treatments & Transcript IDs & Size of Data (MB) \\ \midrule
\texttt{df.cuff} & Cuffdiff & \textit{Homo sapiens} & 2 & 3 & 1200 & 0.2 \\
\texttt{df.deseq} & DESeq2 & \textit{Homo sapiens} & 2, 4 & 4, 2 & 29391 & 54 \\
\texttt{df.edger} & edgeR & \textit{Arabidopsis thaliana} & 2 & 3 & 724 & 0.1 \\ \bottomrule
\end{tabular}
\end{table}
...
\footnotesize
\centering
\caption{``Real-world'' data set statistics. To test the reliability of our package, real data was used from human collections and several plant samples. Each data set is summarized in terms of organism ID, number of experimental samples, experimental conditions, and number of transcripts.}
\label{table2}
\begin{tabular}{@{}lllp{5cm}l@{}}
\toprule
Data & Organism & Samples & Experimental Conditions & Transcript IDs \\ \midrule
Real-world 1 & \textit{Homo sapiens} & 10 & Two treatment dosages taken at two time points and one control sample taken at one time point (two replicates each). & 198002 \\
Real-world 2 & \textit{Malus domestica} & 24 & Two phenotypes taken at four time points (three replicates each). & 63517 \\
Real-world 3 & \textit{Vitis riparia}: bud & 6 & Two conditions (three replicates each). & 59262 \\
Real-world 4 & \textit{Vitis riparia}: shoot-tip (7 days) & 6 & Two conditions (three replicates each). & 17962 \\
Real-world 5 & \textit{Vitis riparia}: shoot-tip (21 days) & 6 & Two conditions (three replicates each). & 19064 \\
\bottomrule
\end{tabular}
\end{table}
...
## With Cuffdiff
```{r,  message=FALSE, fig.align='center', fig.cap='A boxplot example using the \\texttt{vsBoxPlot()} function with \\texttt{cuffdiff} data. In this example, FPKM distributions for each treatment within an experiment are shown in the form of a "box and whisker plot". '}
...
## With DESeq2
```{r,  message=FALSE, fig.align='center', fig.cap='A boxplot example using the \\texttt{vsBoxPlot()} function with \\texttt{DESeq2} data. In this example, FPKM distributions for each treatment within an experiment are shown in the form of a "box and whisker plot". '}
...
## With edgeR
```{r,  message=FALSE, fig.align='center', fig.cap='A boxplot example using the \\texttt{vsBoxPlot()} function with \\texttt{edgeR} data. In this example, CPM distributions for each treatment within an experiment are shown in the form of a "box and whisker plot". '}
...
## With Cuffdiff
```{r, message=FALSE, fig.align='center', fig.cap='A scatterplot example using the \\texttt{vsScatterPlot()} function with \\texttt{Cuffdiff} data. In this visualization, $log_{10}$ comparisons are made of fragments per kilobase of transcript per million mapped reads (FPKM) measurments. The dashed line represents regression line for the comparison.'}
...
## With DESeq2
```{r, message=FALSE, fig.align='center', fig.cap='A scatterplot example using the \\texttt{vsScatterPlot()} function with \\texttt{DESeq2} data. In this visualization, $log_{10}$ comparisons are made of fragments per kilobase of transcript per million mapped reads (FPKM) measurments. The dashed line represents regression line for the comparison.'}
vsScatterPlot(x = 'trt', y = 'untrt', data = df.deseq, type = 'deseq',
              d.factor = 'dex', title = TRUE, grid = TRUE)
```

\newpage

## With edgeR
```{r, message=FALSE, fig.align='center', fig.cap='A scatterplot example using the \\texttt{vsScatterPlot()} function with \\texttt{edgeR} data. In this visualization, $log_{10}$ comparisons are made of counts per million mapped reads (CPM) measurments. The dashed line represents regression line for the comparison.'}
...
## With DESeq2
```{r, message=FALSE, fig.align='center', fig.cap='A scatterplot matrix example using the \\texttt{vsScatterMatrix()} function with \\texttt{DESeq2} data. Similar to the scatterplot function, this visualization allows for all comparisons to be made within an experiment. In addition to the scatterplot visuals, FPKM distributions (histograms) and correlation (Corr) values are generated.'}
vsScatterMatrix(data = df.deseq, d.factor = 'cell', type = 'deseq', comp = NULL,
                title = TRUE, grid = TRUE, man.title = NULL)
```
...
## With Cuffdiff
```{r,  message=FALSE, fig.align='center', fig.cap='A matrix of differentially expressed genes (DEGs) at a given \\textit{p}-value using the \\texttt{vsDEGMatrix()} function with \\texttt{Cuffdiff} data. With this function, the user is able to visualize the number of DEGs ata given adjusted \\textit{p}-value for each experimental treatment level. Higher color intensity correlates to a higher number of DEGs.'}
...
## With DESeq2
```{r,  message=FALSE, fig.align='center', fig.cap='A matrix of differentially expressed genes (DEGs) at a given \\textit{p}-value using the \\texttt{vsDEGMatrix()} function with \\texttt{DESeq2} data. With this function, the user is able to visualize the number of DEGs ata given adjusted \\textit{p}-value for each experimental treatment level. Higher color intensity correlates to a higher number of DEGs.'}
vsDEGMatrix(data = df.deseq, padj = 0.05, d.factor = 'cell', type = 'deseq', 
            title = TRUE, legend = TRUE, grid = TRUE)
```
...
## With edgeR
```{r,  message=FALSE, fig.align='center', fig.cap='A matrix of differentially expressed genes (DEGs) at a given \\textit{p}-value using the \\texttt{vsDEGMatrix()} function with \\texttt{edgeR} data. With this function, the user is able to visualize the number of DEGs ata given adjusted \\textit{p}-value for each experimental treatment level. Higher color intensity correlates to a higher number of DEGs.'}
...
## With DESeq2
```{r,  message=FALSE, fig.align='center', fig.cap='MA plot visualization using the \\texttt{vsMAPLot()} function with \\texttt{DESeq2} data. LFCs are plotted mean counts to determine the variance between two treatments in terms of gene expression. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. Numerical values in parantheses for each legend color indicate the number of transcripts that meet the prior conditions. Triangular shapes represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Dashed lines indicate user-defined LFC values.'}
...
## With edgeR
```{r,  message=FALSE, fig.align='center', fig.cap='MA plot visualization using the \\texttt{vsMAPLot()} function with \\texttt{edgeR} data. LFCs are plotted mean counts to determine the variance between two treatments in terms of gene expression. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. Numerical values in parantheses for each legend color indicate the number of transcripts that meet the prior conditions. Triangular shapes represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Dashed lines indicate user-defined LFC values.'}
...
# Example S7: Creating volcano plots
...
## With Cuffdiff
```{r,  message=FALSE, fig.align='center', fig.cap='A volcano plot example using the \\texttt{vsVolcano()} function with \\texttt{Cuffdiff} data. In this visualization, comparisons are made between the $-log_{10}$ \\textit{p}-value versus the $log_2$ fold change (LFC) between two treatments. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. Numerical values in parantheses for each legend color indicate the number of transcripts that meet the prior conditions. Left and right brackets (< and >) represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Vertical and horizontal lines indicate user-defined LFC and adjusted \\textit{p}-values, respectively.'}
...
## With DESeq2
```{r,  message=FALSE, fig.align='center', fig.cap='A volcano plot example using the \\texttt{vsVolcano()} function with \\texttt{DESeq2} data. In this visualization, comparisons are made between the $-log_{10}$ \\textit{p}-value versus the $log_2$ fold change (LFC) between two treatments. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. Numerical values in parantheses for each legend color indicate the number of transcripts that meet the prior conditions. Left and right brackets (< and >) represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Vertical and horizontal lines indicate user-defined LFC and adjusted \\textit{p}-values, respectively.'}
...
## With edgeR
```{r,  message=FALSE, fig.align='center', fig.cap='A volcano plot example using the \\texttt{vsVolcano()} function with \\texttt{edgeR} data. In this visualization, comparisons are made between the $-log_{10}$ \\textit{p}-value versus the $log_2$ fold change (LFC) between two treatments. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. Numerical values in parantheses for each legend color indicate the number of transcripts that meet the prior conditions. Left and right brackets (< and >) represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Vertical and horizontal lines indicate user-defined LFC and adjusted \\textit{p}-values, respectively.'}
...
# Example S8: Creating volcano plot matrices
Similar to a scatter plot matrix, `vsVolcanoMatrix()` will produce visualizations for all comparisons within your data set. For more information on how the aesthetics are plotted in these visualizations, please refer to the figure caption and Method S1.

## With Cuffdiff
```{r,  message=FALSE, fig.width=8, fig.height=5, fig.align='center', fig.cap='A volcano plot matrix using the \\texttt{vsVolcanoMatrix()} function with \\texttt{Cuffdiff} data. Similar to the \\texttt{vsVolcano()} function, \\texttt{vsVolcanoMatrix()} will generate a matrix of volcano plots for all comparisons within an experiment. Comparisons are made between the $-log_{10}$ \\textit{p}-value versus the $log_2$ fold change (LFC) between two treatments. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. The blue and green numbers in each facet represent the number of transcripts that meet the criteria for blue and green nodes in each comparison. Left and right brackets (< and >) represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Vertical and horizontal lines indicate user-defined LFC and adjusted \\textit{p}-values, respectively.'}
vsVolcanoMatrix(data = df.cuff, d.factor = NULL, type = 'cuffdiff', padj = 0.05, 
...
## With DESeq2
```{r,  message=FALSE, fig.width=8, fig.height=5, fig.align='center', fig.cap='A volcano plot matrix using the \\texttt{vsVolcanoMatrix()} function with \\texttt{DESeq2} data. Similar to the \\texttt{vsVolcano()} function, \\texttt{vsVolcanoMatrix()} will generate a matrix of volcano plots for all comparisons within an experiment. Comparisons are made between the $-log_{10}$ \\textit{p}-value versus the $log_2$ fold change (LFC) between two treatments. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. The blue and green numbers in each facet represent the number of transcripts that meet the criteria for blue and green nodes in each comparison. Left and right brackets (< and >) represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Vertical and horizontal lines indicate user-defined LFC and adjusted \\textit{p}-values, respectively.'}
vsVolcanoMatrix(data = df.deseq, d.factor = 'cell', type = 'deseq', padj = 0.05, 
                x.lim = NULL, lfc = NULL, title = TRUE, legend = TRUE, 
                grid = TRUE, counts = TRUE)
```
...
## With edgeR
```{r,  message=FALSE, fig.width=8, fig.height=5, fig.align='center', fig.cap='A volcano plot matrix using the \\texttt{vsVolcanoMatrix()} function with \\texttt{edgeR} data. Similar to the \\texttt{vsVolcano()} function, \\texttt{vsVolcanoMatrix()} will generate a matrix of volcano plots for all comparisons within an experiment. Comparisons are made between the $-log_{10}$ \\textit{p}-value versus the $log_2$ fold change (LFC) between two treatments. Blue nodes on the graph represent statistically significant LFCs which are greater than a given value than a user-defined LFC parameter. Green nodes indicate statistically significant LFCs which are less than the user-defined LFC parameter. Gray nodes are data points that are not statistically significant. The blue and green numbers in each facet represent the number of transcripts that meet the criteria for blue and green nodes in each comparison. Left and right brackets (< and >) represent values that exceed the viewing area of the graph. Node size changes represent the magnitude of the LFC values (i.e. larger shapes reflect larger LFC values). Vertical and horizontal lines indicate user-defined LFC and adjusted \\textit{p}-values, respectively.'}
...
# Example S9: Creating four way plots
...
## With DESeq2
```{r,  message=FALSE, fig.width=8, fig.height=5, fig.align='center', fig.cap='A four way plot visualization using the \\texttt{vsFourWay()} function with \\texttt{DESeq2} data. In this example, LFCs comparisons between two treatments and a control are made. Blue nodes indicate statistically significant LFCs which are greater than a given user-defined value for both x and y-axes. Green nodes reflect statistically significant LFCs which are less than a user-defined value for treatment y and greater than said value for treatment x. Similar to green nodes, red nodes reflect statistically significant LFCs which are greater than a user-defined vlaue treatment y and less than said value for treatment x. Gray nodes are data points that are not statistically significant for both x and y-axes. Triangular shapes indicate values which exceed the viewing are for the graph. Size change reflects the magnitude of LFC values (i.e. larger shapes reflect larger LFC values). Vertical and horizontal dashed lines indicate user-defined LFC values.'}
vsFourWay(x = 'N052611', y = 'N061011', control = 'N080611', data = df.deseq,
...
## With edgeR
```{r,  message=FALSE, fig.width=8, fig.height=5, fig.align='center', fig.cap='A four way plot visualization using the \\texttt{vsFourWay()} function with \\texttt{edgeR} data. In this example, LFCs comparisons between two treatments and a control are made. Blue nodes indicate statistically significant LFCs which are greater than a given user-defined value for both x and y-axes. Green nodes reflect statistically significant LFCs which are less than a user-defined value for treatment y and greater than said value for treatment x. Similar to green nodes, red nodes reflect statistically significant LFCs which are greater than a user-defined vlaue treatment y and less than said value for treatment x. Gray nodes are data points that are not statistically significant for both x and y-axes.'}
...
## Scatterplots
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsScatterPlot()} function. Time (ms) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively. '}
...
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsScatterMatrix()} function. Time (ms) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively.'}
...
## Box plots
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsBoxPlot()} function. Time (ms) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively. '}
...
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsDEGMatrix()} function. Time (ms) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively.'}
...
## Volcano plots
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsVolcano()} function. Time (ms) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively.'}
...
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsVolcanoMatrix()} function. Time (s) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively.'}
...
## MA plots
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsMAPlot()} function. Time (ms) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively. '}
...
## Four way plots
```{r, echo=FALSE, fig.align='center', fig.cap='Benchmarks for the \\texttt{vsFourWay()} function. Time (ms) distributions were generated for this function using 100 trials for each of the three RNAseq data objects. Cuffdiff, DESeq2, and edgeR example data sets contained 1200, 724, and 29391 transcripts, respectively. '}
