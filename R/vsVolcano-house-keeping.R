...
vo.comp1 <- function(x.lim, padj, lfc, b, g) {
  list(sh1 = paste('lfc < ', round(x.lim[1], 2)),
       ...
       vline3 = geom_vline(xintercept = lfc, color = 'grey32', size = 1,
                           alpha = 0.8, linetype = 'dashed'),
       hline1 = geom_hline(yintercept = -log10(padj), color = 'grey32', size = 1, 
                           alpha = 0.8, linetype = 'dashed'),
...
vo.out.ranker <- function(log2fc, lim) {
  ...
  ifelse(abs(log2fc) < lim, 'sub',
         ifelse(abs(vec2) >= tmp[[4]], 't4',
                ifelse(abs(vec2) >= tmp[[3]] & abs(vec2) < tmp[[4]], 't3',
                       ifelse(abs(vec2) >= tmp[[2]] & abs(vec2) < tmp[[3]], 't2',
                              't1')))) 
}
