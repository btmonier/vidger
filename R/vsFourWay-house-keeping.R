...
.four.comp1 <- function(x.lim, y.lim, padj, all.lab, lfc, b, g, r) {
  list(sh1 = paste('|lfc(x)| > ', round(x.lim[2], 1), 'OR  |lfc(y)| > ', 
       ...
       sh2 = paste(round(x.lim[1], 1), '< lfc(x) <', round(x.lim[2], 1), 
                   ' AND ', round(y.lim[1], 1), '< lfc(y) <', 
                   round(y.lim[2]), 1),
       col1 = paste0('padj(x, y) > ', padj),
       col2 = paste0('padj(x) < ', padj, ', |lfc(x)| > ', lfc, ' |lfc(y)| < ',
                     lfc, ' (', g ,')'),
       col3 = paste0('padj(y) < ', padj, ', |lfc(x)| < ', lfc, ' |lfc(y)| > ',
                     lfc, ' (', r ,')'),
       col4 = paste0('padj(x, y) < ', padj, ', |lfc(x, y)| > ', lfc, ' (', b ,')'),
       vline1 = geom_vline(xintercept = 0, color = 'red3', size = 0.5, 
                           alpha = 0.8, linetype = 'longdash'), 
       vline2 = geom_vline(xintercept = -lfc, color = 'grey32', size = 1, 
                           alpha = 0.8, linetype = 'dashed'),
       vline3 = geom_vline(xintercept = lfc, color = 'grey32', size = 1,
                           alpha = 0.8, linetype = 'dashed'),
       hline1 = geom_hline(yintercept = 0, color = 'red3', size = 0.5, 
                           alpha = 0.8, linetype = 'longdash'),
       hline2 = geom_hline(yintercept = -lfc, color = 'grey32', size = 1,
                           alpha = 0.8, linetype = 'dashed'),
       hline3 = geom_hline(yintercept = lfc, color = 'grey32', size = 1,
                           alpha = 0.8, linetype = 'dashed'),
       x.lab = xlab(bquote(.(all.lab[1]) ~ 'expression relative to' ~ 
                             .(all.lab[3]) ~ 'log'['2'] ~ 'fold change')),
       y.lab = ylab(bquote(.(all.lab[2]) ~ 'expression relative to' ~ 
                             .(all.lab[3]) ~ 'log'['2'] ~ 'fold change'))
...
.four.comp2 <- function(a, b, c, d, e, f){
  list(color = scale_color_manual(name = '', 
                                  ...
                                  labels = c('gry' = a, 'grn' = b, 
                                             'blu' = c, 'red' = d)),
       shape = scale_shape_manual(name = '', 
                                  values = c('tri1' = 17, 'circ' = 16),
                                  labels = c('tri1' = e, 'circ' = f)),
       size = scale_size_manual(name = '', 
                                values = c('sub' = 1,
                                           't1' = 2, 
                                           't2' = 3, 
                                           't3' = 4, 
...
.four.out.ranker <- function(px, py, x.lim, y.lim) {
  ...
  ifelse(abs(px) < x.lim & abs(py) < y.lim, 'sub',
         ifelse(abs(vec3) >= tmp[[4]], 't4',
                ifelse(abs(vec3) >= tmp[[3]] & abs(vec3) < tmp[[4]], 't3',
                       ifelse(abs(vec3) >= tmp[[2]] & abs(vec3) < tmp[[3]], 't2',
                              't1')))) 
}
...
.four.col.ranker <- function(dat, lfc) {
  ...
  ifelse(de_a == TRUE & abs(px) >= lfc & abs(py) >= lfc, 'blu',
         ifelse(abs(px) >= lfc & abs(py) < lfc & 
...
.four.col.counter <- function(dat, lfc) {
  ...
  grn.c <- nrow(dat[which(px >= lfc & py < lfc & de_x == TRUE & de_y == FALSE), ])
  red.c <- nrow(dat[which(px < lfc & py >= lfc & de_x == FALSE & de_y == TRUE), ])
